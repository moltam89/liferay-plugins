...
public class FriendsActivityInterpreter extends BaseSocialActivityInterpreter {
	...
	private String getUserProfileURL(long userId, ServiceContext serviceContext)
		...
		String userName = HtmlUtil.escape(user.getFullName());

		String url = SocialNetworkingUtil.getURLWithUserData(
			serviceContext.getPortalURL()+urlPattern, user);

		return wrapLink(url, userName);
	}
