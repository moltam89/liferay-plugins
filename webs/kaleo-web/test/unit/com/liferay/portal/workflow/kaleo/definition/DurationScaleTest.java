...
public class DurationScaleTest {
	...
	public void testParseValidScales() throws Exception {
		for (String scale : SCALES) {
			DurationScale d = DurationScale.parse(scale);
			Assert.assertEquals(scale, d.getValue());
		}
	}
	...
	public void testValues() throws Exception {
		for (int i = 0; i < SCALES.length; i++) {
			DurationScale d = DurationScale.parse(SCALES[i]);
			Assert.assertEquals(VALUES[i], d.getIntegerValue());
		}
	}
